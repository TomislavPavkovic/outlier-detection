...
if __name__ == "__main__":
    ...
    new_dataset_name = "datasets-ct-liver"
    ...
    for root, dirs, files in os.walk(root_directory):
        for dir in dirs:
            if dir.startswith("00"):
                ...
                if organ_name in summary_excel.columns:
                    background_col_num = summary_excel.columns.get_loc('background')
                    organ_col_num = summary_excel.columns.get_loc(organ_name)
                    organ_label = organ_col_num - background_col_num
                    organ_col = summary_excel[organ_name]
