...
def gen_iterator_training(out_path, out_path_optimized,  dataset, gen_p):
    ...
    for i, data in tqdm(enumerate(loader)):
        ...
        try:
            if len(data_tupels) > 20:
                ...
                create_meshes(data_tupels_optimized)
                data_tupels = []
                data_tupels_optimized = []
            logits = gen.generate_mesh(data)
            logits_optimized = gen.generate_mesh_with_optimization(data)
            data_tupels.append((logits,data, out_path))
            data_tupels_optimized.append((logits_optimized, data, out_path_optimized))
